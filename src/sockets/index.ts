...
import {
  ISteamGamePlayerStat,
  SteamGamePlayerStat,
} from "../models/SteamGamePlayerStat";
import {
  ISteamAchievements,
  SteamAchievements,
} from "../models/SteamAchievements";
...
export async function saveProfile(
  ...
  try {
    ...
    const steamCountry = profileChain.data.get("steamCountry");
    if (steamCountry && steamCountry.__kind == "SingleValue") {
      profile.steamCountry = steamCountry.value;
    }
...
export async function fetchFriendList(
  ...
  try {
    ...
    const url = `https://api.steampowered.com/ISteamUser/GetFriendList/v0001/?key=${
      ...
      testingUser || profile.steamId || profile.identity
    }&relationship=friend`;
    const friendList = await honeycomb
      .http()
      .get(url)
      .then((res) => {
  ...
  try {
    ...
    tree.setLeaves(
      await orm.em
        ...
        .then((rows) =>
          rows.map((row) => row.toJSON() as SteamOwnedCollectible)
        )
    );
    ...
    const url = `https://steamcommunity.com/inventory/${
      testingUser || profile.steamId || profile.identity
    }/${game.app_id || 730}/2?l=english&key=${config.steam_api_key || ""}`;
...
export async function fetchGamePlayerStats(
  ...
  try {
    ...
    const stats = profileObj.entity<ISteamGamePlayerStat>(
      "SteamGamePlayerStat"
    );
    stats.setLeaves(
      await orm.em
    ...
    const gamePlayerStat = await honeycomb
      .http()
      .get(url)
      .then((res) => res?.playerstats as ISteamGamePlayerStatApi)
      .catch((e) => {
        console.error({ e });
        return;
      });
    ...
    for (let statRaw of gamePlayerStat.stats) {
      ...
      const storedStat = stats.values.find(
        (t) => t.name == statRaw.name && t.app_id == game.app_id
      );
...
export async function fetchGameAchievements(
  ...
  try {
    ...
    const achievements =
      profileObj.entity<ISteamAchievements>("SteamAchievements");
    ...
    const gamePlayerStat = await honeycomb
      .http()
      .get(url)
      .then((res) => res?.playerstats as ISteamPlayerAchievementsApi)
      .catch((e) => {
        console.error({ e });
        return;
      });
    ...
    for (let achievementRow of gamePlayerStat.achievements) {
      ...
      const storedAchievement = achievements.values.find(
        (t) => t.name == achievementRow.apiname && t.app_id == game.app_id
      );
...
export async function fetchOwnedGamesDetails(
  ...
  try {
    ...
    const ownedGameList = await axios
      .get(url)
      .then((res) => (res.data.response?.games || []) as ISteamOwnedGamesApi[]);
...
export async function ensureSteamUsersInDb(orm: MikroORM) {
  ...
  (
    ...
    if (v.steamId) idsSet.add(v.steamId);
  });
  (
    await orm.em.find(
      Profile,
  ...
  for (let index = 0; index < ids.length / 100; index++) {
    const url = `https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=${
      config.steam_api_key
    }&steamids=${ids.slice(index * 100, (index + 1) * 100).join(",")}`;
    const userSteamSummaries = await axios.get(url).then(
      (res) =>
        res.data.response?.players as {
          steamid: string;
          avatar: string;
          personaname: string;
          realname: string;
    ...
    for (let summary of userSteamSummaries) {
      const steamUser = new SteamUser(
        summary.steamid || "",
        summary.avatar || "",
        summary.realname || summary.personaname || "",
...
export async function ensureSteamGameCollectiblesAssetsInDb(orm: MikroORM) {
  ...
  (
    ...
    if (v.class_id && v.app_id) addClassToApp(v.app_id, v.class_id);
  });
  (
    await orm.em.find(
      SteamAssetClassInfo,
      {
        class_id: {
          $in: Array.from(getAllClassIds().values()),
        },
      },
      {
        fields: ["class_id"],
  ...
  for (let [app_id, classIds] of appClassIdsMap) {
    const url = `https://api.steampowered.com/ISteamEconomy/GetAssetClassInfo/v1/?key=${
      config.steam_api_key
    }&appid=${app_id}&class_count=${classIds.size}&${Array.from(
      classIds.values()
    )
      .map((v, i) => `classid${i}=${v}`)
      .join("&")}&language=en`;
    const gameCollectibleAssets = await axios
      .get(url)
      .then((res) => res.data.result as IAssetClassInfoApi)
      .catch((e) => ({ success: false }));
    for (let [resKey, asset] of Object.entries(gameCollectibleAssets)) {
      if (!asset.classid || ["success", "error", "message"].includes(resKey))
        continue;
